...
| **Security hygiene** | tracked PR state; scheduled/main history baseline | common credential/private-key secret shapes are absent from the checked scope | a formal guarantee that no secret can exist; provider-side secret scanning remains complementary |
| **DATA reproducibility** | `data_module/pyproject.toml` + lockfile | Poetry can reproduce the committed DATA dependency resolution contract under the pinned lock-generation toolchain | availability of heavy DATA artifacts, Solidity compiler matrix, GPU runtime, or accepted R4 physical roots |
...
| **DATA** | DATA unit tests in its own Poetry environment | ingestion/Slither/representation pipelines, protected/local R4 artifacts | heavy physical data is not guaranteed in a fresh clone |
...
`data_module/` owns a separate Poetry environment. Its lockfile is part of the public reproducibility contract and should be generated by Poetry from `data_module/pyproject.toml`, committed, and checked in CI.
...
- **history baseline scan:** full-history scan on `main`, scheduled runs, or explicit manual execution.

The repository-level scanner intentionally targets high-signal forms such as private-key assignments, PEM private keys, common provider credential URLs, GitHub tokens, AWS access-key IDs, and mnemonic assignments. It is a bounded engineering control, not a mathematical proof of absence.
