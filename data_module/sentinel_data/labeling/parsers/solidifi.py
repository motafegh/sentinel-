"""SolidiFI label parser — Task 3.5.

Reads Stage 1 preprocessed meta.json files for the solidifi source,
extracts the injection folder from original_path, maps it through the
crosswalk YAML, and writes one .labels.json per contract.

Label source: original_path field in meta.json
  e.g. repo/buggy_contracts/Re-entrancy/buggy_3.sol  →  folder = Re-entrancy
       folder → crosswalk → canonical class

Output: data/labels/solidifi/<sha256>.labels.json
"""
...
from sentinel_data.labeling.schema import class_names, class_index

log = logging.getLogger("sentinel_data.labeling.solidifi")
...
def _extract_folder(original_path: str) -> str | None:
    """Extract the injection folder name from original_path.

    original_path = "repo/buggy_contracts/<FOLDER>/buggy_N.sol"
    Returns <FOLDER>, or None if the path doesn't match the expected structure.
    """
...
def _build_labels_json(
    ...
    """Build the full .labels.json dict for one contract."""
...
def label_source(
    ...
    *,
    ...
    injection folder to a canonical class via the crosswalk, and writes
    ...
    out_dir = output_dir if output_dir is not None else data_dir / "labels" / "solidifi"

    if not prep_dir.exists():
        raise FileNotFoundError(
            f"{prep_dir} not found. Run `sentinel-data preprocess --source solidifi` first."
        )
