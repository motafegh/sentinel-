...
| `cli.py` | 1053 | **The single user-facing surface.** `sentinel-data <stage> [--dry-run] [--config] [--source]` + `sentinel-data run [--from-stage STAGE]`. 9 stages + 1 utility (`freshness`). |
| `ingestion/` | 524 lines (4 files) | Stage 1a — pull raw contracts from heterogeneous source types. See `ingestion/README.md`. |
| `preprocessing/` | 1,493 lines (9 files) | Stage 1b — flatten, compile, dedup, normalize, segment. See `preprocessing/README.md`. |
...
| `splitting/` | 922 lines (4 files) | Stage 5 — 4 splitters + dedup_enforcer + leakage_auditor + NonVuln cap. See `splitting/README.md`. |
...
| `export/` | 995 lines (7 files + format_schema/) | Stage 7 — 4-writer sharded export (graphs, tokens, labels, metadata) + orchestrator + consumer-facing API. See `export/README.md`. |

**Sub-total: ~11,400 lines of Python** across 66 files (excluding `__pycache__` and sub-package `__init__.py` files; includes the package-level `__init__.py`).
...
`cli.py:_STAGE_FN` (line 761-772) is the dispatch table. Each stage function returns `None` on success, or an int exit code (currently only `verify` does this — non-zero on `--strict` mode FAIL).
...
| `sentinel-data label [--source NAME] [--dry-run]` | 3 | **STUB** — "NOT IMPLEMENTED — implement in Stage 3" (`cli.py:228-234`). Merger runs from Python today. | No |
...
| `sentinel-data export [--config CONFIG] [--dataset-version NAME] [--split-version N] [--shard-size N] [--output-dir PATH] [--dry-run]` | 7 | `cli.py:_run_export` — calls `export.chunk_export()` to produce sharded graphs/tokens/labels/metadata + manifest | No |
...
|-------|-----------|------|
| `cli.py` | ↔ | Top-level orchestrator; `_STAGE_FN` dispatch table at line 761-772 |
...
- `test_representation/` — `test_thin_adapter.py`, `test_byte_identical_regression.py`, `test_orchestrator.py`, `test_solidifi_fixes.py`, `test_13_issue_preservation.py`, `test_emits_fixture.py` (no `__init__.py` — pytest still discovers)
...
- `test_analysis/` — `test_analysis.py` (analysis tools coverage)
