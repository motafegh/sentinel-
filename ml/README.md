...
╚══════════════════════════════════════════════════════════════════════════════════╝
  ...
  │        │                                                                    │
  │        └──► create_splits.py ──► train/val/test indices (.npy)             │
  │             (stratified split)    47,966 / 10,278 / 10,279                 │
  └─────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
  ┌─────────────────────────────────────────────────────────────────────────────┐
  ...
  │                                                                             │
  │  DualPathDataset ──► SentinelModel ──► BCEWithLogitsLoss / FocalLoss        │
  │                       │                                                     │
  │                       ├── GNNEncoder (3-layer GAT + edge embeddings)        │
  │                       ├── TransformerEncoder (CodeBERT + LoRA r=8)          │
  │                       └── CrossAttentionFusion ──► Classifier               │
  │                                                                             │
  │  Checkpoint (.pt) + thresholds (.json) ──► ml/checkpoints/                 │
  └─────────────────────────────────────────────────────────────────────────────┘
                                        │
                       ┌────────────────┴────────────────┐
                       ▼                                  ▼
  ┌──────────────────────────────┐     ┌──────────────────────────────────────┐
  │  THRESHOLD TUNING            │     │  MODEL REGISTRY                      │
  │  tune_threshold.py           │     │  promote_model.py                    │
  │  Grid sweep 0.05–0.95        │     │  MLflow Staging → Production         │
  │  per class on val split      │     └──────────────────────────────────────┘
  │  → *_thresholds.json         │
  └──────────────────────────────┘
                                        │
                                        ▼
  ...
  │  { "source_code": "..." }                                                  │
  │        │                                                                    │
  │        ├── InferenceCache lookup  (key: md5 + FEATURE_SCHEMA_VERSION)       │
  ...
  │        │                                                                    │
  │        ├── sigmoid(logits) → probabilities [B, 10]                         │
  │        ├── apply per-class thresholds from *_thresholds.json               │
  │        │                                                                    │
  │        └── DriftDetector  (KS test every 50 requests)                      │
  │              → Prometheus: sentinel_drift_alerts_total                      │
  │                                                                             │
...
│                      │               │                              │
│  Slither AST         │               │  CodeBERT tokenizer          │
│   → graph.x  [N, 8]  │               │   → input_ids    [B, 512]    │
│   → edge_index[2,E]  │               │   → attn_mask    [B, 512]    │
│   → edge_attr  [E]   │               │                              │
│                      │               │  TransformerEncoder          │
│  GNNEncoder          │               │   CodeBERT (124M frozen)     │
│   edge_emb  [E,16]   │               │   + LoRA r=8 α=16 (~295K     │
│   conv1  [N,8]→[N,64]│               │     trainable on Q+V)        │
│   conv2 [N,64]→[N,64]│               │                              │
...
└──────────┬───────────┘                              │
           ...
           └────────────────┬─────────────────────────┘
            ...
            │                              │
            │  Project nodes [N,64]→[N,256]│
            │  Project tokens[B,512,768]   │
            │           →   [B,512,256]    │
            │                              │
            │  to_dense_batch → [B,n,256]  │
            │  (zero-pad shorter graphs)   │
            │                              │
            │  Node→Token attention:       │
            │    Q=nodes  [B,n,256]        │
            │    K=V=tokens[B,512,256]     │
            │    mask=PAD token positions  │
            │    → enriched nodes [B,n,256]│
            │                              │
            │  Token→Node attention:       │
            │    Q=tokens [B,512,256]      │
            │    K=V=nodes[B,n,256]        │
