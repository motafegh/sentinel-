...
The most dangerous findings fall into three categories: 
1. **Silently wrong predictions in production** due to missing model configuration forwarding (`gnn_jk_mode`, `lora_target_modules`) and broken sliding-window tokenization that omits `[CLS]/[SEP]` special tokens; 
2. **Misleading training metrics** where the GNN gradient share metric monitors only the projection layer, not the GNN backbone, meaning collapse detection can report healthy shares while the actual GNN is frozen; and 
3. **Data pipeline fragility** including path-based hashing that creates duplicate entries, cache integrity checks that validate only 1 of 44,470 entries, and a `weights_only=True` token loading path that crashes on every token file.
...
The most critical invariant for any ML system is that preprocessing at inference must be identical to preprocessing at training. Three mismatches were identified: 
1. Sliding-window tokenization omits `[CLS]/[SEP]` on non-first windows (produces garbage).
2. `gnn_jk_mode` is not forwarded in inference (produces wrong embeddings).
3. `lora_target_modules` deserialization bug in `tune_threshold.py` (produces wrong thresholds). 

All three produce silently wrong predictions without any error signal.
...
| **H3** | HIGH | `trainer.py:425` | Shared grad clipping negates per-group LR multipliers |
| **H4** | HIGH | `trainer.py:301` | `pos_weight` has no upper cap; gradient spikes for rare classes |
| **H5** | HIGH | `trainer.py:405` | `pos_weight` applied to aux losses amplifies GNN instability |
| **H6** | HIGH | `trainer.py:693` | Unexpected `state_dict` keys silently dropped |
| **H7** | HIGH | `trainer.py:418` | NaN poisons entire accumulation window |
| **H8** | HIGH | `trainer.py:472` | Fewer scheduler steps = OneCycleLR never reaches min LR |
| **H9** | HIGH | `gnn_encoder.py:379` | No `edge_attr` bounds validation |
| **H10** | HIGH | `gnn_encoder.py:140` | JK attention rank-1 scoring limits expressivity |
| **H11** | HIGH | `gnn_encoder.py:417` | `REVERSE_CONTAINS` embedding untrained at start |
| **H12** | HIGH | `graph_extractor.py:418` | `CFG_NODE_CALL` `call_target_typed` defaults to "safe" |
| **H13** | HIGH | `graph_extractor.py:417` | `return_ignored` always 0.0 at statement level |
| **H14** | HIGH | `dual_path_dataset.py:208` | Cache integrity checks only 1 of 44,470 entries |
| **H15** | HIGH | `dual_path_dataset.py:195` | No schema version in cache pkl |
| **H16** | HIGH | `graph_schema.py:215` | `public` and `external` map to same visibility value |
| **H17** | HIGH | `predictor.py:144` | `weights_only=False` enables arbitrary code execution |
| **H18** | HIGH | `api.py` | No authentication on `/predict` endpoint |
| **H19** | HIGH | `tokenizer.py:348` | `imap` reordering corrupts failed hash logging |
| **M1** | MEDIUM | `fusion_layer.py:84` | Default `node_dim=64` wrong (should be 128) |
| **M2** | MEDIUM | `fusion_layer.py:200` | `clamp(min=1)` masks degenerate zero-node graphs |
| **M3** | MEDIUM | `fusion_layer.py:123` | 512-to-128 single-layer compression bottleneck |
| **M4** | MEDIUM | `trainer.py:1012` | Aux warmup starts at 1/3 target, not 0 as documented |
| **M5** | MEDIUM | `trainer.py:333` | `autocast(device)` breaks if device includes index |
| **M6** | MEDIUM | `trainer.py:827` | No bias/norm exemption from weight decay |
| **M7** | MEDIUM | `gnn_encoder.py:157` | No temperature in JK softmax |
| **M8** | MEDIUM | `gnn_encoder.py:280` | Phase 2 `heads=1` limits multi-pattern learning |
| **M9** | MEDIUM | `transformer_encoder.py:86` | Docstring says 295K params but r=16 gives ~590K |
| **M10** | MEDIUM | `transformer_encoder.py:126` | Hard-coded model name, not configurable |
| **M11** | MEDIUM | `graph_extractor.py:757` | `type_id` normalization round-trip is fragile |
| **M12** | MEDIUM | `ast_extractor.py:311` | Binary labels hardcoded to 0 |
| **M13** | MEDIUM | `dual_path_dataset.py:295` | Hardcoded token shape check rejects valid shorter sequences |
| **M14** | MEDIUM | `cache.py` | No size bound; unbounded disk consumption |
| **M15** | MEDIUM | `promote_model.py` | No minimum F1 gate for model promotion |