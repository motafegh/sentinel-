...
**Module:** data_module
**Owner:** Ali
**Status:** STARTING — no code changes yet, read-only investigation first
...
| 0b | Spec-file coverage/quality audit + fixes | ✓ | ✓ (Ali asked) | ✓ | ✓ | pending | Found 6 HIGH + MED/LOW coverage gaps; added Method 0 (criteria), Method 8 (parser faithfulness), control arm, blind re-check, pre-committed decision rule, dependency map, provenance; CLAUDE.md gained Concerns F/G/H + stat-reporting + provenance. Notes: `~/.claude/scratch/dive_specfiles_coverage_audit_20260618.md`. |
| 8 | Method 8 — Parser faithfulness | ✓ | ✓ (Ali asked) | ✓ | ✓ (corrected) | pending | **VERDICT: Parser faithful for all 7 DIVE-sourced classes.** 22,073/22,073 perfect match for EB, RE, IUO, TS, TOD, UR. DoS 2,655 mismatch = intentional DoS+RE co-occurrence patch (not bug). 257 contracts dropped (compile failures, <1.7% per class). Mapping 1:1, no collisions. Self-critique: initially misdiagnosed patch as parser bug; corrected after Ali pointed to MEMORY.md + patch script. Deliverable: `findings/08_parser_faithfulness.md` + `scripts/parser_faithfulness.py`. |
| 2 | Method 2 — Folder↔CSV per-contract identity | ✓ | ✓ (Ali asked) | ✓ | ✓ | pending | **0/178,640 mismatches.** 100% per-contract agreement for all 8 classes across all 22,330 contracts. Zero-label contracts (2,686) confirmed correct. Folder symlinks are a 100% faithful reproduction of CSV. Chain of trust (CSV→folders→parser→labels) now fully validated for Phase 1. Deliverable: `findings/02_folder_csv_agreement.md` + `scripts/verify_folder_csv_agreement.py`. |
| 1 | Method 1 — DIVE methodology | ✓ | ✓ ("go on") | ✓ | ✓ | pending | **DIVE labels are fully automated tool-derived consensus, not ground truth.** 6 tools (MAIAN, Mythril, Semgrep, Slither, Solhint, VeriSmart) with Power-based voting + post-hoc filtering (14-25% FP correction reported). No manual verification. Authors: labels are "systematically derived, high-confidence annotations rather than manually verified ground truth." Multi-label by design. Only verified contracts (2016-2024). Key implication: prior ~5% TP rate is expected for tool-derived labels, not data corruption. This MUST inform Method 0 criteria. Deliverable: `findings/01_dive_methodology.md`. |
| 0 | Method 0 — Freeze TP/FP/BORDERLINE criteria | ✓ | ✓ (Ali asked) | in progress | partial | — | **IN PROGRESS.** REACHABLE bar for EB/RE (proposed, awaiting confirmation). BORDERLINE bucket defined. Practical floor ≥10% TP **confirmed** (2026-06-19). Batch 1 (10 contracts) done, 0 TP found. Batch 2 sizing under review. Tools: slither_reference.md, aderyn_reference.md. Handoff: `METHOD_0_HANDOFF.md`. |
| — | — | — | — | — | — | — | Next Method not yet started — awaiting Ali's go for batch 2 |
