...
**Scope:** canonical R4 execution status on `main`. Historical G0–G7 evidence remains valid for the immutable `sentinel-r4-vnext-v1` lineage. Phase 8 is still `IN_PROGRESS`: the local repaired-v2 corpus/publication is physically accepted and the bounded GPU smoke passed, but the all-positive supervised objective and unresolved long-contract selector prevent a credible full run. The 100-epoch retrain is not authorized.
...
| 8 | `phases/09_PHASE_8_EXISTING_MODEL_RETRAINING.md` | IN_PROGRESS | G7 | G8 | Physical `sentinel-r4-vnext-v2` acceptance passed for 22,540 contracts / 67,620 files and the bounded repaired-data GPU smoke passed. G8 remains open: all 899 effective loss cells are positive-only, no confirmed negatives exist, and the measured four-window control has material target-code omission. Full 100-epoch training is not authorized. |
...
| Physical repaired-v2 evidence | ACCEPTED LOCAL / GENERATED | The Git-ignored local lineage contains 22,540 bound contracts / 67,620 files with zero missing/invalid payloads; acceptance passed all 20 checks and the bounded CUDA smoke passed. Generated artifacts are not present in a fresh clone. |
...
The completed rebuild procedure is preserved in the local-data handoff and gate re-audit. The current authority is `runs/2026-08-15_PHASE8_repaired_data_acceptance_and_launch_decision.md`. No old pretraining handoff may be used to launch the 100-epoch job.
