...
>
> **Out of scope:** Preparing the NEXT training run. That is documented in the two companion files in `data_module/temp/live_plans/`:
> - `run_12_to_13_handoff_2026-06-14.md` (287 lines) — the 6-step Run N → Run N+1 handoff workflow
> - `run13_plan_2026-06-14.md` (336 lines) — the 4-fixes details for Run 13
>
...
```bash
PYTHONPATH=. ml/.venv/bin/python -m ml.scripts.calibrate_temperature \
    --checkpoint ml/checkpoints/GCB-P1-Run<N>-<tag>-YYYYMMDD_best.pt \
    --out ml/calibration/temperatures_run<N>.json
# Outputs:
#   ml/calibration/temperatures_run<N>.json      (10-class T values)
#   ml/calibration/temperatures_run<N>_stats.json  (ECE before/after)
#   ml/calibration/temperatures_run<N>_ece_comparison.png  (bar chart)
```

**Output schema (per `ml/calibration/temperatures_run9.json`):** `{class_name: T_c, ...}` — one T per class.
...
ls -la ml/calibration/temperatures_run<N>.json
...
**`ml/scripts/check_contamination.py` is STALE and only checks SmartBugs vs BCCC (using v9/v10 paths, not v3).** The script never compared the benchmark contracts against the actual v3 training set — so we have been reporting benchmark numbers on **mostly training data**.
...
PYTHONPATH=. ml/.venv/bin/python -m ml.scripts.benchmark_run9_smartbugs \
    --checkpoint ml/checkpoints/GCB-P1-Run<N>-<tag>-YYYYMMDD_best.pt \
    --verbose > ml/reports/Run<N>_smartbugs_FULL_$(date +%Y%m%d).log 2>&1
...
PYTHONPATH=. ml/.venv/bin/python -m ml.scripts.benchmark_run9_solidifi \
    --checkpoint ml/checkpoints/GCB-P1-Run<N>-<tag>-YYYYMMDD_best.pt \
    --verbose > ml/reports/Run<N>_solidifi_FULL_$(date +%Y%m%d).log 2>&1
```
