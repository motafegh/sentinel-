...
# BCCC class columns
class_cols = ["Class11:Reentrancy", "Class10:IntegerUO", "Class06:UncheckedReturn", 
              "Class01:ExternalBug", "Class08:CallToUnknown", "Class03:MishandledException",
              "Class02:GasException", "Class09:DenialOfService", "Class12:NonVulnerable"]
...
for cls in class_cols:
    ...
    print(f"  {cls:<22}: {len(ok_with_cls):>3} / {total_with_cls:>5} = {100*len(ok_with_cls)/max(total_with_cls,1):.1f}%")

print("\nEXCEPTION contracts per class:")
for cls in class_cols:
    ex_with_cls = [cid for cid in ex_ids if sample.get(cid, {}).get(cls, "0") == "1"]
    total_with_cls = sum(1 for cid in sample if sample[cid].get(cls, "0") == "1")
    print(f"  {cls:<22}: {len(ex_with_cls):>3} / {total_with_cls:>5} = {100*len(ex_with_cls)/max(total_with_cls,1):.1f}%")

# Check BCCC vulnerability hits in OK results
